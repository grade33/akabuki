...
;// CONCATENATED MODULE: ./src/scripts/modules/select.js
function initSelect() {
  document.querySelectorAll('.select').forEach(function (select) {
    var head = select.querySelector('.select__head');
    var body = select.querySelector('.select__body');
    var optionCol = Array.from(body.children);
    var item = body.querySelector('.is-selected .select__item') || body.querySelector('.select__item');
    ...
    var selectIsOpen = function selectIsOpen() {
      return body.classList.contains('is-open');
    };
    var openCloseHandler = function openCloseHandler(e) {
      if (!(e.type === 'click' || e.type === 'keydown' && e.key === 'Enter')) return;
      if (selectIsOpen()) {
        selectClose();
        return;
      }
    ...
    optionCol.forEach(function (option) {
      var changeHandler = function changeHandler(e) {
        if (!(e.type === 'click' || e.type === 'keydown' && e.key === 'Enter')) return;
        head.innerHTML = null;
        var prevSelectedOption = body.querySelector('.is-selected');
        prevSelectedOption.classList.remove('is-selected');
        option.classList.add('is-selected');
        var currentSelectedItem = option.querySelector('.select__item');
...
;// CONCATENATED MODULE: ./src/scripts/modules/accordion.js
